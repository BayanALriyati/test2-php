...
if(preg_match("/^(?=.*[A-Za-z])(?=.*\d)(?=.*[@$!%*#?&])[A-Za-z\d@$!%*#?&]{8,20}$/" ,$_POST['password']) && !empty($_POST['password'])) {
  $seven =1;
 }
else {
...
if ($one ==1 && $two ==1 && $three ==1 && $four ==1 && $five ==1 && $six ==1 && $seven==1 && $eight ==1 && $nine ==1 ){
   $sql="INSERT INTO validation (ID,FirstName,	MiddleName,LastName,FamilyName,Email,Password,Mobile,DateOfBirth) 
